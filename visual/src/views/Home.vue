  ...
  <div class="screen-container" :style="containerStyle">
    <header class="screen-header">
      ...
      <span class="logo">
        <!-- <img :src="logoSrc" alt="" /> -->
        <a
          :class="[titleColor ? 'active' : '']"
          href="https://www.wbrong.top"
          title="wbrong.top"
          target="_blank"
          >wbrong.top</a
        >
      ...
      <div class="title-right">
        ...
        <span class="datetime">{{ currentLocalDate }}</span>
      </div>
    </header>
...
export default {
  ...
  data() {
    return {
      titleColor: true,
      currentLocalDate: new Date().toLocaleString().replace(/\//g, "-"),
      timer: null,
      // 定义每一个图表的全屏状态
      fullScreenStatus: {
        trend: false,
  ...
  methods: {
    ...
    currentTime() {
      ...
      this.timer = setInterval(() => {
        this.currentLocalDate = new Date().toLocaleString().replace(/\//g, "-");
      }, 800);
    },
  },
  components: {
